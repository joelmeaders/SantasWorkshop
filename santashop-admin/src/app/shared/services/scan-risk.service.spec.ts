...
describe('ScanRiskService', () => {
		...
		TestBed.configureTestingModule({
			providers: [
				ScanRiskService,
				{
					...
					useValue: {
						collection: vi.fn().mockReturnValue({ read, readMany }),
					},
				},
			],
		});
			...
			of([
				{
					...
					latestRiskOn: {
						toDate: (): Date => new Date('2026-12-02'),
					},
					originalCheckInOn: {
						toDate: (): Date => new Date('2026-12-03'),
					},
				},
			]),
		);
	...
	it('normalizes individual attempts and optional check-ins', async () => {
		...
		await expect(
			firstValueFrom(service.attempts(2026, 'customer-1')),
		).resolves.toMatchObject([
			{
				scannedOn: new Date('2026-12-04'),
				priorEventOn: new Date('2026-12-03'),
			},
		]);
		await expect(
			firstValueFrom(service.checkIn('customer-1')),
		).resolves.toMatchObject({
