...
import { IError, Registration } from '../../../../../../../santashop-models/src/public-api';
...
export class SubmitPageService {
	...
	public async submitRegistration(): Promise<void> {
		...
		try {
			const registration = await firstValueFrom(this.preregistrationService.userRegistration$);
			const completionResult = await this.completeRegistration(registration);

			// This exception should never be hit since the function doesn't return false
			if (!completionResult) throw new Error('Cloud Function error in completeRegistrationFunction()');
	 ...
	 */
	private async completeRegistration(registration: Registration, attempts = 0): Promise<boolean> {
