...
import { Firestore, DocumentReference, DocumentData } from '@angular/fire/firestore';
...
describe('FireRepoLite', () => {
  ...
  it('read<T>(): should make expected call', async () => {
    ...
    // Assert
    expect(spy).toHaveBeenCalledOnceWith('registrations', '12345', <any>'uid');
  });

  it('readMany<T>(): should make expected call', async () => {
    // Arrange
    const spy = spyOn(fireRepoBase, 'readMany').and.returnValue(of([mockData]));

    // Act
    service.readMany<IRegistration>('registrations', undefined, 'uid');

    // Assert
    expect(spy).toHaveBeenCalledOnceWith('registrations', undefined, <any>'uid');
  });

  it('add<T>(): should make expected call', async () => {
    // Arrange
    const spy = spyOn(fireRepoBase, 'add').and.returnValue(of({} as DocumentReference<IRegistration>));
  ...
  it('addById<T>(): should make expected call', async () => {
    // Arrange
    const spy = spyOn(fireRepoBase, 'addById').and.returnValue(of({} as DocumentReference<IRegistration>));

    // Act
    service.addById('registrations', '12345', mockData);

    // Assert
    expect(spy).toHaveBeenCalledOnceWith('registrations', '12345', mockData);
  });

  it('update<T>(): should make expected call', async () => {
    // Arrange
    const spy = spyOn(fireRepoBase, 'update').and.returnValue(of({} as DocumentReference<DocumentData>));

    // Act
    service.update('registrations', '12345', mockData, true);

    // Assert
    expect(spy).toHaveBeenCalledOnceWith('registrations', '12345', mockData, true);
  });
