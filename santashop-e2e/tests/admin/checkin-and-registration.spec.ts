...
test.describe('check-in and staff registration operations', () => {
	...
	test('CHECKIN-010 completes a valid manual-code check-in', async ({
		page,
		seedRegistration,
	}) => {
		...
		await expect(
			page.getByText('Casey Checkin', { exact: true }),
		).toBeVisible();
		await page.getByText('Yes, check in', { exact: true }).click();
		await expect(page).toHaveURL(/\/admin\/checkin\/confirmation$/);
		await expect(
			page.getByText('Give the shopper 1 coupon.'),
		).toBeVisible();
	});

	test('CHECKIN-005 presents a scanned registration for staff review before normal confirmation', async ({
		page,
		seedRegistration,
	}) => {
		await seedRegistration(registration);
		...
		await expect(page).toHaveURL(/\/admin\/checkin\/review$/);
		await expect(
			page.getByText('Review all registration information.'),
		).toBeVisible();
		await expect(
			page.getByText('Test Child', { exact: true }),
		).toBeVisible();
		await expect(
	...
	test('CHECKIN-004 keeps an unknown manual code out of check-in and offers staff lookup recovery', async ({
		page,
	}) => {
		await signInAdminViaUi(page, defaultAdminAccount());
		...
		await expect(page).not.toHaveURL(
			/\/admin\/checkin\/(review|confirmation)/,
		);
		await alert
		...
		await expect(
			page.getByRole('heading', {
				name: 'Find a registration',
				exact: true,
			}),
		).toHaveText('Find a registration');
	});
	...
	test('CHECKIN-007A blocks a likely accidental rescan immediately', async ({
		page,
		seedRegistration,
	}) => {
		await seedRegistration({
			...registration,
			uid: 'duplicate-checkin-e2e',
			code: 'E2EDUP01',
			hasCheckedIn: true,
		});
		await signInAdminViaUi(page, defaultAdminAccount());
		...
		await expect(page).toHaveURL(
			/\/admin\/checkin\/duplicate\/duplicate-checkin-e2e$/,
		);
		await expect(
			page.getByText('Possible accidental double scan'),
		).toBeVisible();
		await expect(
			page.getByText('Do not issue tickets or coupons.'),
		).toBeVisible();
		await expect(
			page.getByRole('heading', { name: 'Current blocked attempt' }),
		).toBeVisible();
		await expect(
			page.getByRole('heading', { name: 'Original successful check-in' }),
		).toBeVisible();
		await expect(
			page.getByText('Code ending in UP01', { exact: true }),
		).toBeVisible();
		await expect(
			page.getByText(/seconds before this attempt/),
		).toBeVisible();
		await expect(
		...
		await expect(
			page
				.getByText(/coupons?/i)
				.filter({ hasNotText: 'Do not issue tickets or coupons.' }),
		).toHaveCount(0);
	});

	test('CHECKIN-007B adds a late duplicate to the admin scan risk review', async ({
		page,
		seedRegistration,
	}) => {
		await seedRegistration({
		...
		await expect(
			page.getByText('Contact a DSCS member to resolve this attempt.'),
		).toBeVisible();
		await expect(
			page.getByRole('heading', { name: 'Current blocked attempt' }),
		).toBeVisible();
		...
		await expect(
			page.getByText('Code ending in ATE1', { exact: true }),
		).toBeVisible();
		await expect(
			page.getByText(/seconds before this attempt/),
		).toBeVisible();
		...
		await expect(page).toHaveURL(
			/\/admin\/stats\/scan-risk\/late-duplicate-e2e$/,
		);
		await expect(
			page.getByText('Successful check-in', { exact: true }),
		).toBeVisible();
		await expect(
			page.getByText('Suspicious duplicate scan', { exact: true }),
		).toBeVisible();
	});
	...
	test('CHECKIN-007C keeps a cancelled code blocked, then permits a re-registered replacement exactly once', async ({
		...
		await expect(
			page.getByText('Canceled registration code', { exact: true }),
		).toBeVisible();
		await expect(
			page.getByText('Contact a DSCS member to resolve this attempt.'),
		).toBeVisible();
		...
		await expect(
			page.getByRole('heading', { name: 'Registration canceled' }),
		).toBeVisible();
		await expect(
			page.getByText('Code ending in LD01', { exact: true }),
		).toBeVisible();
		await expect(
			page.getByText(/seconds before this attempt/),
		).toBeVisible();
		...
		await expect(page.getByText(/Late duplicate 1/)).toBeVisible();
		await page.getByText('Casey Checkin', { exact: true }).click();
		await expect(page.getByText('Canceled', { exact: true })).toHaveCount(
			2,
		);
		await expect(
			page.getByText('Suspicious duplicate scan', { exact: true }),
	...
	test('SCAN-RISK-001 paginates current-season risk summaries and keeps each customer timeline newest first', async ({
		...
		await page.goto('/admin/stats/scan-risk');
		await expect(
			page.getByText('Mixed Attempts', { exact: true }),
		).toBeVisible();
		await expect(
			page.getByText('4 risk attempt(s)', { exact: true }),
		).toBeVisible();
		await expect(
			page.getByText('Late duplicate 2', { exact: true }),
		).toBeVisible();
		await expect(
			page.getByText('Canceled code 2', { exact: true }),
		).toBeVisible();
		await expect(
			page.getByText('Prior Season', { exact: true }),
		).toHaveCount(0);
		await expect(
		...
		await page
			...
			.click();
		await expect(
			page.getByText('Paged Customer 20', { exact: true }),
		).toBeVisible();

		await page.getByText('Mixed Attempts', { exact: true }).click();
		await expect(page).toHaveURL(
			/\/admin\/stats\/scan-risk\/scan-mixed-e2e$/,
		);
		const timeline = page.locator(
			'ion-list[aria-label="Customer scan timeline"] ion-item',
		);
		await expect(timeline).toHaveCount(4);
		await expect(timeline.nth(0).getByRole('heading')).toHaveText(
		...
		await expect(timeline.nth(1).getByRole('heading')).toHaveText(
			'Canceled',
		);
	...
	test('CHECKIN-006 checks in an edited registration after staff changes its appointment', async ({
		...
		await seedDateTimeSlots([
			{
				id: 'checkin-original-slot',
				programYear: E2E_PROGRAM_YEAR,
				dateTime: e2eDateTime(12, 15, 16),
				maxSlots: 8,
			},
			{
				id: 'checkin-edited-slot',
				programYear: E2E_PROGRAM_YEAR,
				dateTime: e2eDateTime(12, 16, 16),
				maxSlots: 8,
			},
		]);
		await seedRegistration({
			...registration,
			uid: 'edited-checkin-e2e',
			code: 'E2EEDIT1',
		});
		await signInAdminViaUi(page, defaultAdminAccount());
	...
	for (const action of ['edit', 'add'] as const) {
		test(`CHECKIN-CHILD-${action} persists a child-only change at check-in`, async ({
			page,
			request,
			seedRegistration,
		}) => {
			...
			if (action === 'edit') {
				await page
					.getByRole('heading', { name: 'Test Child', exact: true })
					.click();
				await page
			...
			await fillIonicInput(
				page,
				'ion-modal ion-input[formControlName="firstName"]',
				'Updated',
			);
			if (action === 'add') {
				await fillIonicInput(
					page,
					'ion-modal ion-input[formControlName="lastName"]',
					'Child',
				);
				await fillIonicInput(
					page,
			...
			await modal
				.getByRole('radio', { name: 'Girls', exact: true })
				.click();
			await modal
				.getByRole('button', { name: 'Save Child', exact: true })
				.click();
			await expect(modal).toHaveCount(0);
			await page.getByText('Yes, check in', { exact: true }).click();
			await expect(page).toHaveURL(/\/admin\/checkin\/confirmation$/);
			await expect(
				page.getByText(
					`Give the shopper ${action === 'add' ? '2 coupons' : '1 coupon'}.`,
					{ exact: true },
				),
			).toBeVisible();
			...
			await expect
				.poll(async () => {
					const response = await request.get(
						`${E2E_FIRESTORE_EMULATOR_URL}/v1/projects/${E2E_PROJECT_ID}/databases/(default)/documents/editedregistrations/${uid}`,
						{
					...
					return (document.fields?.children?.arrayValue?.values ?? [])
						.map(
							(child: {
								mapValue: {
									fields: {
							...
							}) => child.mapValue.fields.firstName.stringValue,
						)
						.sort();
				})
	...
	test('CHECKIN-011 lets staff cancel a submitted registration from review and blocks its superseded code', async ({
		...
		await expect(confirm).toContainText(
			'Deleting this reservation cannot be undone',
		);
	...
	test('ADMIN-PRE-001 creates a staff pre-registration with a selected slot', async ({
		page,
		seedDateTimeSlots,
	}) => {
		await seedDateTimeSlots([
			{
				id: 'pre-register-slot',
				programYear: E2E_PROGRAM_YEAR,
				dateTime: e2eDateTime(12, 16, 16),
				maxSlots: 8,
			},
		...
		await page
			.locator('ion-modal')
			.getByText('SNAP', { exact: true })
			.click();
		await page
			.locator('ion-select[formControlName="dateTimeSlot"]')
			.click();
		...
		await modal
			.locator('ion-input[formControlName="dateOfBirth"]')
			.evaluate(
				(element, dateOfBirth) =>
	...
	test('ADMIN-REG-001 completes a staff-created walk-in registration and check-in', async ({
		page,
	}) => {
		await signInAdminViaUi(page, defaultAdminAccount());
