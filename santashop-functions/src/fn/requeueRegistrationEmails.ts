...
export default async (): Promise<void> => {
  ...
  const registrationDocQuery = admin.firestore()
    .collection(`${COLLECTION_SCHEMA.registrations}`)
      .where('programYear', '==', 2021)
      .orderBy('registrationSubmittedOn');

  const registrations = await registrationDocQuery.get().then(snapshotDocs => {
  ...
  registrations.forEach(registration => {
    ...
    const emailDocRef = admin.firestore()
      .doc(`${COLLECTION_SCHEMA.tmpResendRegistrationEmails}/${registration.uid}`);
