import { SendTemplatedEmailCommand, SendTemplatedEmailCommandOutput, SESClient, SESClientConfig } from '@aws-sdk/client-ses';
...
const createReminderEmailCommand = (messageDetails: any, toEmailAddress: string) => {
...
export default async (docChange: functions.firestore.QueryDocumentSnapshot) => {
  ...
  const sendReminderEmailCommand = createReminderEmailCommand(messageDetails, email);
