...
const MANAGED_ENV_KEYS = [
	'SANTASHOP_LOAD_TEST_MODE',
	'TEST_SANTASHOP_EMAIL_TRANSPORT',
	'TEST_SANTASHOP_EMAIL_VPC_CONNECTOR',
...
describe('config.functions.cjs', () => {
	...
	it('builds test Functions config from TEST values and omits empty optional values', () => {
		...
		expect(config['SANTASHOP_EMAIL_TRANSPORT']).toBeUndefined();
		expect(config['SANTASHOP_EMAIL_VPC_CONNECTOR']).toBeUndefined();
	...
	it('requires explicit test-only load mode and strips credentials in that mode', () => {
		process.env['SANTASHOP_LOAD_TEST_MODE'] = 'true';
		const config = configFunctions.buildFunctionsConfig('test');
		expect(config['AWS_ACCESS_KEY_ID']).toBeUndefined();
		expect(config['AWS_SECRET_ACCESS_KEY']).toBeUndefined();
		expect(config['SANTASHOP_EMAIL_TRANSPORT']).toBe('sink');
		expect(config['SCHEDULED_DATETIME_SLOT_COUNTERS']).toBe('*/5 * * * *');
		expect(() => configFunctions.buildFunctionsConfig('prod')).toThrow(
			'restricted',
		);
		process.env['SANTASHOP_LOAD_TEST_MODE'] = 'yes';
	...
	it('rejects a production config when an AWS credential is missing', () => {
		setManagedEnv({ ...FUNCTIONS_ENV_KEYS });
		delete process.env['PROD_AWS_SECRET_ACCESS_KEY'];

		expect(() => configFunctions.buildFunctionsConfig('prod')).toThrow(
			'Missing required environment variable: PROD_AWS_SECRET_ACCESS_KEY',
		);
	});
	...
	it.each([
		...
		['PROD_AWS_ACCESS_KEY_ID', 'your-aws-access-key-id'],
		['PROD_AWS_SECRET_ACCESS_KEY', 'your-aws-secret-access-key'],
	...
	it('renders a quoted env file for the selected project', () => {
		...
		expect(fileText).toContain('AWS_SECRET_ACCESS_KEY="test-secret-key"');
		expect(fileText).not.toContain('SANTASHOP_EMAIL_TRANSPORT=');
