...
describe('config.functions.cjs', () => {
	...
	it('builds test Functions config from TEST values and omits empty optional values', () => {
		...
		expect(config['SANTASHOP_EMAIL_VPC_CONNECTOR']).toBe(
	...
	it('renders a quoted env file for the selected project', () => {
		...
		expect(fileText).not.toContain('AWS_ACCESS_KEY_ID=');
		expect(fileText).not.toContain('AWS_SECRET_ACCESS_KEY=');
		expect(fileText).toContain('SANTASHOP_EMAIL_TRANSPORT="sink"');
