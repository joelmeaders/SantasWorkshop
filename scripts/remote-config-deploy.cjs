...
const {
	inspectGateway,
	GATEWAY_FUNCTION,
} = require('./remote-config-gateway.cjs');
...
const deploy = async (
	...
	if (process.argv[2] !== mode)
		throw new Error(
			'Release argument must match SANTASHOP_FUNCTIONS_DEPLOY.',
		);
	const selected = (name) => {
		const value = env[name] ?? 'true';
		if (!['true', 'false'].includes(value))
			throw new Error('Invalid deployment selection: ' + name);
		return value === 'true';
	};
	const deployFunctions = selected('SANTASHOP_DEPLOY_FUNCTIONS');
	const deployRules = selected('SANTASHOP_DEPLOY_RULES');
	if (!deployFunctions && !deployRules)
	...
	if (deployFunctions) {
		...
		const base = [
			'deploy',
			'--config',
			'firebase.functions-deploy.json',
			'--project',
			projectId,
		];
		firebase([
		...
		for (let attempt = 0; attempt < 12; attempt++) {
			try {
				...
				gateway = { problems: [error.message] };
			}
			if (attempt < 11)
				await new Promise((resolve) => setTimeout(resolve, 10_000));
		}
		if (!gateway || gateway.problems.length || !gateway.gatewayUri)
			throw new Error(
				`Private gateway deployment is not ready: ${gateway?.problems.join(' ')}`,
			);
		env[`${getModePrefix(mode)}_SANTASHOP_REMOTE_CONFIG_GATEWAY_URL`] =
			gateway.gatewayUri;
		node(['config.functions.cjs', mode]);
